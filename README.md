# Excel-UK-Road-Accident-Analysis-Dashboard
This project analyzes and visualizes UK road traffic accident data through an interactive dashboard to uncover trends, severity distribution, and key factors affecting road safety.  Time-based analysis examines patterns by year, month, and day, revealing seasonal trends, peak periods, and long-term changes in accident frequency.


![Image alt](https://github.com/kumar98rishav-oss/Excel-UK-Road-Accident-Analysis-Dashboard/blob/7e8fc329b91a8759bf41893b530a73492812917f/UK%20Dashboard%20SS.png)

# Excel Sales Dashboard 📊

## 📌 Project Overview
This Excel dashboard analyzes sales performance across regions, products, and time.

## 🧩 Business Questions Answered
- Which region generates the highest revenue?
- Monthly sales trend analysis
- Top-performing products
- Profit vs Sales comparison

## 🛠 Tools Used
- Microsoft Excel
- Pivot Tables
- Power Query
- Slicers & Charts
- Advanced Excel formulas

## 📊 Dashboard Preview
![Dashboard]([UK Dashboard SS.png](https://github.com/kumar98rishav-oss/Excel-UK-Road-Accident-Analysis-Dashboard/blob/main/UK%20Dashboard%20SS.png)

## 📁 Files Included
- Sales_Dashboard.xlsx → Interactive Excel dashboard
- Raw_Data.xlsx → Original dataset

## 📈 Key Insights
- North region contributes 42% of total revenue
- Product A shows highest profit margin
- Sales peak during Q3

## 🔗 How to Use
Download the Excel file and enable editing & macros (if applicable).
